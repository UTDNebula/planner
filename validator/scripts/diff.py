...
def htmldiff(
    ...
    bsOld = BeautifulSoup(oldContent, features="lxml").find(
        "div", attrs={"id": "bukku-page"}
    )
    bsNew = BeautifulSoup(newContent, features="lxml").find(
        "div", attrs={"id": "bukku-page"}
    )
...
def create_ticket(
    ...
    if issueType == "R":
        ...
        description += (
            "See attachment for the course(s) that have been renamed/added/removed:\n"
            + str(coursesImpacted)
            + "\n\n"
        )
...
if __name__ == "__main__":
    ...
    for majorReqJson in os.scandir(major_json_path):
        data = json.loads(
            open(
                f"/home/runner/work/planner/planner/validator/degree_data/"
                + majorReqJson.name,
                "r",
            ).read()
        )
