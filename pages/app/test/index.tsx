...
export default function TopAndSidebar(): JSX.Element {
  ...
  // x-data: open=true
  return (
    <div className="antialiased min-h-screen bg-blue-900">
      <nav
        className={`absolute transform duration-200 z-10 w-96 bg-white text-white min-h-screen p-3 ${
          open ? 'translate-x-0 ease-in' : '-translate-x-full ease-out opacity-0'
        }`}
      >
        {/* :className="{'translate-x-0 ease-in':open === true, '-translate-x-full ease-out opacity-0': open === false}"> */}
        <div className="flex justify-between">
          ...
          >
            {/* :className="{'opacity-0':open === false, 'opacity-100': open === true}"
                @click="open = false"> */}
            <svg
        ...
        <div className="pt-72">
          <section className="">
            <div
              className="flex items-center justify-center pr-24"
              //   x-data="{ circumference: 2 * 22 / 7 * 60 }"
            >
              ...
              <span
                className="absolute text-4xl text-blue-900 font-semibold font-sans pb-8 pl-8 drop-shadow-xl"
      ...
      <div
        className={`relative transform duration-200 flex items-center z-0 bg-gray-500 last:align-right ${
          open ? 'translate-x-96 ease-in' : '-translate-x-0 ease-out'
        } `}
      >
        {/* :className="{'translate-x-96 ease-in':open === true, '-translate-x-0 ease-out': open === false}"> */}
        <button
        ...
        >
          {/* :className="{'opacity-0':open === true, 'opacity-100': open === false}"
            @click="open = true"> */}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-6 w-6"
